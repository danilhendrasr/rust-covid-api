...
pub async fn index_handler() -> HttpResponse {
    ...
    struct WantedObject {
        penambahan: types::source_api::Penambahan,
        total: types::source_api::Total,
    }
    ...
    match resp {
        ...
        Ok(x) => match x.json::<CovidAPIResponse>().await {
            ...
            Ok(y) => {
                let to_return: types::HandlerResponseTemplate<HandlerResponseMainData> =
                    types::HandlerResponseTemplate::<HandlerResponseMainData> {
                        ok: true,
                        data: HandlerResponseMainData {
